...
# print(df_numeric)
# 只选取特定年的数据,再删除该列
df_numeric = df_numeric[df_numeric["Year"] == Year]
...
plt.barh(importance_otu['Feature'][:20][::-1], importance_otu['Importance'][:20][::-1])
...
# gini_importance = pd.DataFrame({'Feature': df_numeric.drop(columns='Score').columns, 'Importance': rf_model.feature_importances_})
...
# 提取变量重要性
importance_otu_all = pd.DataFrame({
...
# 分别按照三种重要性从大到小排序
importance_otu_gini = importance_otu_all.sort_values(by='Gini_Importance', ascending=False)
importance_otu_mse = importance_otu_all.sort_values(by='MSE_Reduction', ascending=False)
# importance_otu_permutation = importance_otu_all.sort_values(by='Permutation_Importance', ascending=False)
print(importance_otu_gini)
print(importance_otu_mse)
# print(importance_otu_permutation)
# 输出前几行变量重要性
print(importance_otu_all.head())
...
plt.barh(importance_otu_gini['Feature'][:30][::-1], importance_otu_gini['Gini_Importance'][:30][::-1])

# MSE Reduction
plt.subplot(1, 2, 2)
plt.title('MSE Reduction')
plt.barh(importance_otu_mse['Feature'][:30][::-1], importance_otu_mse['MSE_Reduction'][:30][::-1], color='orange')

# Permutation Importance
# temp commet
# plt.subplot(1, 3, 3)
# plt.title('Permutation Importance')
# plt.barh(importance_otu_permutation['Feature'][:30][::-1], importance_otu_permutation['Permutation_Importance'][:30][::-1], color='green')
